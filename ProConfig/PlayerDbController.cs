...
namespace ProConfig
{
    public class PlayerDbController
    {
        ...
        public void DownloadCfg(string playerName, string steamCfgPath)
        {
        ...
        public bool CheckConfigExists(string playerName)
        {
            using (MySqlConnection cn = conn.GetConnection())
            {
                string query = "SELECT Config FROM proPlayer WHERE PlayerName=@playerName AND Config>''";
                MySqlCommand cmd = new MySqlCommand(query, cn);
                cmd.Parameters.Add("@playerName", MySqlDbType.VarChar).Value = playerName;
                cn.Open();
                var reader = cmd.ExecuteReader();
        ...
        public bool CheckAutoexecExists(string playerName)
        {
            using (MySqlConnection cn = conn.GetConnection())
            {
                string query = "SELECT Autoexec FROM proPlayer WHERE PlayerName=@playerName AND Autooexec >''";
                MySqlCommand cmd = new MySqlCommand(query, cn);
                cmd.Parameters.Add("@playerName", MySqlDbType.VarChar).Value = playerName;
                cn.Open();
                var reader = cmd.ExecuteReader();
        ...
        public bool CheckVideoExists(string playerName)
        {
            using (MySqlConnection cn = conn.GetConnection())
            {
                string query = "SELECT Video FROM proPlayer WHERE PlayerName=@playerName AND Video >''";
                MySqlCommand cmd = new MySqlCommand(query, cn);
                cmd.Parameters.Add("@playerName", MySqlDbType.VarChar).Value = playerName;
                cn.Open();
                var reader = cmd.ExecuteReader();
